...
const MainLayout = styled.div`
  ...
  min-height: 100vh; /* Make sure the main layout takes at least the full height of the viewport */
  background-color: #ffffff;
  justify-content: space-between; /* This will push your footer to the bottom */
`;
...
const StyledButton = styled.button<{ disabled: boolean }>`
  ...
  &:hover {
    background-color: ${({ disabled }) =>
      disabled ? "#666" : "#fc8742"}; // Slightly change color on hover if not disabled
  }
`;
...
const Content = styled.main`
  flex: 1; /* This allows the content area to expand and fill the available space, pushing the footer down */
  padding: 2rem;
  display: flex;
  flex-direction: column;
  align-items: center;
...
const Section = styled.section`
  width: 100%;
  max-width: 1200px;
  margin: 2rem auto; /* Adjusted top margin, assuming it was larger before */
  text-align: center;
...
const H1_5 = styled.h2`
  ...
  margin-bottom: 1rem; /* Adjusted from a potentially larger value */
  /* Other styles remain unchanged */
`;
...
const App: React.FC = () => {
  ...
  const handleCardClick = (cardId: string) => {
    if (cardId == "analyze") {
      if (analyze) setAnalyze(false);
      else setAnalyze(true);
    } else {
      setAnalyze(false);
    }
    if (cardId == "summarize") {
      if (summarize) setSummarize(false);
      else setSummarize(true);
    } else {
      setSummarize(false);
    }
    if (cardId == "suggest") {
      if (summarize) setSuggest(false);
  ...
  return (
    <MantineProvider>
      <MainLayout>
        <StyledHeader>
          <NavigationBar>
            <Image radius="md" h={30} w="auto" fit="contain" src="./logo.png" />
          </NavigationBar>
        </StyledHeader>
        <ScrollArea.Autosize>
          <LoadingProvider>
        ...
        <Footer>© 2024 Mike.AI. All rights reserved.</Footer>
      </MainLayout>
    </MantineProvider>
  );
};
